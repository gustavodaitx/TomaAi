package br.com.tomai.ui.theme

import androidx.compose.ui.graphics.Color

// Paleta oficial TomaAí.
val PrimaryLight = Color(0xFF0288D1)
val OnPrimaryLight = Color(0xFFFFFFFF)
val PrimaryContainerLight = Color(0xFF97F0FF)
val OnPrimaryContainerLight = Color(0xFF001F24)

val PrimaryDark = Color(0xFF83CFFF)
val OnPrimaryDark = Color(0xFF00363D)
val PrimaryContainerDark = Color(0xFF004F58)
val OnPrimaryContainerDark = Color(0xFF97F0FF)

// Paleta Secundária - Ardósia Suave
val SecondaryLight = Color(0xFF005B9F)
val OnSecondaryLight = Color(0xFFFFFFFF)
val SecondaryContainerLight = Color(0xFFCCE8ED)
val OnSecondaryContainerLight = Color(0xFF051F23)

val SecondaryDark = Color(0xFF9CCBEB)
val OnSecondaryDark = Color(0xFF1B3438)
val SecondaryContainerDark = Color(0xFF324B4F)
val OnSecondaryContainerDark = Color(0xFFCCE8ED)

// Paleta Terciária - Índigo Acalmador
val TertiaryLight = Color(0xFF525E7D)
val OnTertiaryLight = Color(0xFFFFFFFF)
val TertiaryContainerLight = Color(0xFFDAE2FF)
val OnTertiaryContainerLight = Color(0xFF0E1A37)

val TertiaryDark = Color(0xFFBAC6EA)
val OnTertiaryDark = Color(0xFF24304D)
val TertiaryContainerDark = Color(0xFF3B4664)
val OnTertiaryContainerDark = Color(0xFFDAE2FF)

// Neutros e Superfícies
val BackgroundLight = Color(0xFFF8FAFC)
val OnBackgroundLight = Color(0xFF191C1D)
val SurfaceLight = Color(0xFFFBFDFD)
val OnSurfaceLight = Color(0xFF191C1D)
val SurfaceVariantLight = Color(0xFFDBE4E6)
val OnSurfaceVariantLight = Color(0xFF3F484A)
val OutlineLight = Color(0xFF6F797B)

val BackgroundDark = Color(0xFF191C1D)
val OnBackgroundDark = Color(0xFFE1E3E3)
val SurfaceDark = Color(0xFF191C1D)
val OnSurfaceDark = Color(0xFFE1E3E3)
val SurfaceVariantDark = Color(0xFF3F484A)
val OnSurfaceVariantDark = Color(0xFFBFC8CA)
val OutlineDark = Color(0xFF899294)

// Cores Semânticas
val ErrorLight = Color(0xFFBA1A1A)
val OnErrorLight = Color(0xFFFFFFFF)
val SuccessGreen = Color(0xFF2E7D32)
val BlueHealthPrimary = Color(0xFF0288D1)
val BlueHealthDark = Color(0xFF005B9F)
val GreenSuccess = Color(0xFF2E7D32)
val SuccessCardLight = Color(0xFFE8F5E9)

// Cores semânticas com contraste suficiente em fundos claros e escuros.
val PendingLight = Color(0xFF8A4B00)
val PendingContainerLight = Color(0xFFFFDCC2)
val PendingDark = Color(0xFFFFB77C)
val PendingContainerDark = Color(0xFF653900)
val SuccessDark = Color(0xFF8BDCAB)
val SuccessContainerLight = Color(0xFFB7F1C8)
val SuccessContainerDark = Color(0xFF00522D)
